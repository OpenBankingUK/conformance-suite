...
func TestServerConfigGlobalPostInvalid(t *testing.T) {
	testCases := []struct {
		...
		{
			name:               `request_object_signing_alg_missing`,
			expectedBody:       `{"error":"request_object_signing_alg: cannot be blank."}`,
			expectedStatusCode: http.StatusBadRequest,
			config: GlobalConfiguration{
				SigningPrivate:          privateKey,
				SigningPublic:           publicKey,
		...
		{
			name:               `request_object_signing_alg_invalid`,
			expectedBody:       `{"error":"request_object_signing_alg: must be a valid value."}`,
			expectedStatusCode: http.StatusBadRequest,
			config: GlobalConfiguration{
				SigningPrivate:                privateKey,
				SigningPublic:                 publicKey,
				...
				XFAPIFinancialID:              "123",
				Issuer:                        "https://modelobankauth2018.o3bank.co.uk:4101",
				RequestObjectSigningAlgorithm: "INVALID",
				ResourceIDs: model.ResourceIDs{
					AccountIDs: []model.ResourceAccountID{
						{AccountID: "account-id"},
					},

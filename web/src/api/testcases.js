...
const PUBLIC_TESTCASES_URL = '/api/run/:runID';
...
export default {
  ...
  /**
   * Call GET /public/api/run/:runID
   */
  async computePublicTestCases(runID) {
    const response = await api.get(PUBLIC_TESTCASES_URL.replace(':runID', runID));
    const data = await response.json();
